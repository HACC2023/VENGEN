...
const ThreadDiscussionItem = ({ item }) => {
  ...
  return (
    <a href={`/thread/${item._id}`} style={{ textDecoration: 'none', color: 'black' }}>
      <Container style={containerStyle} className="d-block thread-discussion-item mb-4">
        ...
        <Row style={{ padding: 5 }}>
          <div id="segmented-buttons-wrapper" style={{ paddingLeft: '12px' }}>
            <div id="segmented-like-button">
              ...
              <h5 style={{ marginBottom: '0px' }}>{item.likes}</h5>
            </div>
            <div id="segmented-dislike-button">
              <HandThumbsDown id="thumbs-down" size={25} color="black" />
              <h5 style={{ marginBottom: '0px' }}>{item.dislikes}</h5>
            </div>
          </div>
        </Row>
      </Container>
...
ThreadDiscussionItem.propTypes = {
...
export default ThreadDiscussionItem;
